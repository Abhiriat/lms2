...
export function DashboardLayout({ sx, cssVars, children, slotProps, layoutQuery = 'lg', }) {
    ...
    const renderHeader = () => {
        ...
        const headerSlots = {
            ...
            leftArea: (_jsxs(_Fragment, { children: [_jsx(MenuButton, { onClick: onOpen, sx: { mr: 1, ml: -1, [theme.breakpoints.up(layoutQuery)]: { display: 'none' } } }), _jsx(NavMobile, { data: navData, open: open, onClose: onClose, workspaces: _workspaces })] })),
            rightArea: (_jsxs(Box, { sx: { display: 'flex', alignItems: 'center', gap: { xs: 0, sm: 0.75 } }, children: [_jsx(Searchbar, {}), _jsx(LanguagePopover, { data: _langs }), _jsx(NotificationsPopover, { data: _notifications }), _jsx(AccountPopover, { data: _account })] })),
        };
    ...
    , { 
         ...
         * @Sidebar
         *************************************** */
        sidebarSection: _jsx(NavDesktop, { data: navData, layoutQuery: layoutQuery, workspaces: _workspaces }), 
        /** **************************************
         * @Footer
         *************************************** */
